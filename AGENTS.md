...
- 以下处置基于 DSH 0.1.2 系列的 Windows workspace-write 环境；DSH 版本升级后应重新核对当前官方机制。Git 命令先按原样正常执行；只有原命令已经失败，且根据错误信息或已核实的环境特征能确认原因是 workspace-write 沙箱限制时，才按 DSH 官方逐次提权机制重试完全相同的命令，并等待机主批准。不得为处理此类沙箱限制而使用 `--no-verify`、关闭或替换 Git/LFS 钩子、手工传递令牌、修改 Git/DSH 配置，或把整个会话切换为 `danger-full-access`；失败原因不明、逐次提权不可用或提权后仍失败时，停止并如实报告。
