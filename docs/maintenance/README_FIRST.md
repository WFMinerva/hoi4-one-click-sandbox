...
> 已于2026-08-23按v2.9状态更新（Steam 工坊、开发分支与正式标签均已推送完成）；2026-09-05 升格为当前状态权威并修正分支与上一基准表述；2026-09-05 机主拍板长期分支策略：main 为唯一长期工作分支，正式版本由 Git 标签固定。
...
- 状态说明：v2.9 正式包 SHA-256 `c0c1f4319944c98ffc2f658dc9c0024499ed4b83f1dff63401279a58951477e7`（34 文件）；标签 `v2.9` 指向 `1304b3f` 并已推送；Steam 工坊物品 `3767025052` 已上传并更新简介（Manifest `3014636165754564201`，2026-08-23 22:44 日志 `Upload finished ... : OK`）。工作线 HEAD 随维护前移，以 `origin/main` 实测为准，本文不登记必然过期的当前 HEAD 值。
...
- Git 现状（2026-09-05 核实）：长期分支策略已由机主拍板——`main` 为唯一长期工作分支，正式版本由 Git 标签固定；标签 `v2.9` 指向提交 `1304b3f`，该提交现可从 main 到达。大型或高风险任务使用短期任务分支，验收后合回 main；不维持第二条永久开发线。本次迁移以 `bd6b7cf` 为快进基准：main 自 `7cafc81` 经 `--ff-only` 快进，带入的 69 个提交包含已正式发布的 v2.8、v2.9 MOD 全部内容，无合并提交，不引入未发布游戏内容。此后 main HEAD 以 `origin/main` 实测为准，本文不登记必然过期的 HEAD 值。旧开发分支 `agent/cross-machine-governance` 已停止作为日常工作线，暂时保留、本轮不删除，三台机器全部迁移并完成至少一次正常维护后再议去留。正式标签按发版门禁创建，自 v2.4 起 GitHub Release 挂正式包 ZIP 附件（ASCII 文件名 + 中文显示名）便于维护取用；唯一下载入口仍以 Steam 工坊为准。
