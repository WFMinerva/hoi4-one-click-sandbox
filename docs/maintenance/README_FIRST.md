...
> 本文已于 2026-07-28 按仓库现状更新（此前版本引用旧交接包的 `01_当前稳定版/` 等目录，已不存在）。
...
标签注意事项：旧 `v2.0` 标签误落在源码导入之前，不能作为 MOD 源码使用；真正的 v2.0 源码基准为提交 `9593154`，补充标签为 `v2.0-source-baseline`。`v2.1` 标签内的游戏内容与当前一致，但标签内旧构建器会误装 v2.0 基准文档；当前仓库构建器已修复，今后标签必须在构建器、基准文档和源码全部定稿后创建。
...
- 旧 `v2.0` 标签只作错误历史引用；恢复 v2.0 源码时使用 `v2.0-source-baseline`。
...
修改任何进入发布包的文件（MOD 内容、`docs/baseline/`、许可证文件等）后，必须重新运行 `python tools/build_release.py` 生成新包与校验文件，并验证通过。构建器会固定 ZIP 元数据、验证内嵌逐文件清单；缺少当期基准文档时直接拒绝构建。
