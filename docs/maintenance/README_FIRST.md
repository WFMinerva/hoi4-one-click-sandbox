...
- 状态说明：v2.8 测试线 v2.8-test1…test6 静态门禁全程全绿、K3 逐批复检通过；玩家「一般通」对 v2.8-test5 完成 8 项点验（1–5、8 无异常，6 MIO 项个别机构差异系玩家游戏停在 1.19.1、表按 1.19.2 生成所致，7 无法观测非缺陷），总体「可发包」；#7 修复经 K3 终审通过。正式包由 `tools/build_release.py` 构建；Git 标签与 GitHub Release 按发版门禁执行，Steam 工坊物品 `3767025052` 上传后回填 Manifest。
