...
> **本文是当前状态（版本、发布状态、Manifest、分支概况）的唯一人类可读权威入口**，表述带核实日期；分支、提交、标签等机器可查询事实以 Git 本身为权威。根 `AGENTS.md` 为薄入口（任务路由、修改红线与验证/发布底线）。
> 已于2026-08-23按v2.9状态更新（Steam 工坊、开发分支与正式标签均已推送完成）；2026-09-05 升格为当前状态权威并修正分支与上一基准表述。

## 当前版本状态

- 正式版本：**v2.9**（按年份研究、政治外交与法案控制、装备设计/库存、全国建设和大型设施扩展；CWTools 本地门禁；见 `docs/baseline/` 与 `CHANGELOG.md`）。
- 上一稳定基准：**v2.8**（Steam 工坊物品 `3767025052`，Manifest `6381266305931940230`；情报机构、特殊科研、MIO 与装备奖励补全）；更早基准与版本演进见 `docs/maintenance/功能与版本交接单.md`。
- 状态说明：v2.9 正式包 SHA-256 `c0c1f4319944c98ffc2f658dc9c0024499ed4b83f1dff63401279a58951477e7`（34 文件）；标签 `v2.9` 指向 `1304b3f` 并已推送；Steam 工坊物品 `3767025052` 已上传并更新简介（Manifest `3014636165754564201`，2026-08-23 22:44 日志 `Upload finished ... : OK`）。开发分支 `agent/cross-machine-governance` 的 HEAD 随维护前移，以 `origin/agent/cross-machine-governance` 实测为准，本文不登记当前 HEAD（本轮修改前核实基准为 `ac8d33a`，2026-09-05）。
...
- Git 现状（2026-09-05 核实）：标签 `v2.9` 指向提交 `1304b3f`；该提交可从开发分支 `agent/cross-machine-governance` 到达，不能从当前 `main`（`7cafc81`）到达；两分支相差 68 个提交（当日 `git rev-list --left-right --count main...agent/cross-machine-governance` 实测 0/68）。此为带核实日期的现状快照，不是永久规则；长期分支策略另行决定。正式标签按发版门禁创建，自 v2.4 起 GitHub Release 挂正式包 ZIP 附件（ASCII 文件名 + 中文显示名）便于维护取用；唯一下载入口仍以 Steam 工坊为准。
...
2. 根目录 `AGENTS.md`：薄入口——自然语言任务路由、修改红线与验证/发布底线。
