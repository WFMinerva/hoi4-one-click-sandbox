...
> 本文已于2026-08-20按v2.8状态更新（发布流水线执行中：正式包构建/标签/Release/工坊上传）。

## 当前版本状态

- 正式版本：**v2.8**（在v2.7稳定基准之上完成情报机构「本地化训练中心」升级补齐、支援/补给船tooltip修复、史前喷气BBA反门、特殊科研42组官方文案、核能石墨/重水选择、巡洋潜艇MtG模块与陆海火箭22组互斥装备奖励补全、MIO资金规模拉满与功能最强路线、陆军9项正面generic装备加成、共享MIO表机构守卫；见 `docs/baseline/` 与 `CHANGELOG.md`）。
- 上一稳定基准：v2.7（Steam工坊物品 `3767025052`，Manifest `8538688044863448269`；一键骷髅师、开发占领区与补给站加速）。
- 状态说明：v2.8 测试线 v2.8-test1…test6 静态门禁全程全绿、K3 逐批复检通过；玩家「一般通」对 v2.8-test5 完成 8 项点验（1–5、8 无异常，6 MIO 项个别机构差异系玩家游戏停在 1.19.1、表按 1.19.2 生成所致，7 无法观测非缺陷），总体「可发包」；#7 修复经 K3 终审通过。正式包由 `tools/build_release.py` 构建（SHA-256 `e34f0e462b07e5b7d0f810655eb7a31069a893ae5cb00d2eb60860c074c8b6e9`，31 文件）；Git 标签 `v2.8`（提交 `d4085e7`）与 GitHub Release 已完成；Steam 工坊物品 `3767025052` 已上传 v2.8（Manifest `6381266305931940230`）。
- 实机状态：本机 v2.8-test6 会话加载干净（setup.log 9 行 PRC_OCS 加载记录、error.log 0 条 MOD 错误、text.log 0 字节）；51 用例自检轮与玩家 test6 复测回执按发布节奏补办。
...
```powershell
Get-FileHash -Algorithm SHA256 dist\开局一键爽玩_v2.8_正式版.zip
```
